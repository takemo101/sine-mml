...
| `--midi-out` | - | MIDIデバイスID/名前（v3.0新機能） | - |
| `--midi-channel` | - | MIDIチャンネル（1-16）（v3.0新機能） | 1 |
| `--midi-list` | - | MIDIデバイス一覧を表示（v3.0新機能） | false |
...
v3.0で追加された連符機能を使用すると、複数の音符を指定した拍数に均等に配分できます。（v3.0新機能）

### 概要

`{...}n` 構文で、n個の音符をn拍子に収めます。これにより、3連符、5連符などの複雑なリズムを簡単に表現できます。

### 基本構文

| 構文 | 説明 | 例 |
|-----|------|-----|
| `{...}n` | n連符 | `{CDE}3`（3連符：3音を1拍に収める） |
| `{...}n:m` | ベース音長指定 | `{CDE}3:2`（2分音符ベースの3連符） |
...
# 4連符
sine-mml play "{CDEF}4"
# 4つの音符を1拍に収める

# 5連符
sine-mml play "{CDEFG}5"
# 5つの音符を1拍に収める

# 2分音符ベースの3連符
sine-mml play "{CDE}3:2"
# 2分音符の長さを3等分

# 連符内で休符も使用可能
sine-mml play "{CR E}3"
# 3連符内に休符を含む
```
...
```bash
# ネストした連符
sine-mml play "{{CDE}3 FG}5"
# 内側の3連符と外側の音を5連符として配分
```
...
```bash
# ループ内で連符を使用
sine-mml play "[{CDE}3 G]2"
# 3連符+Gを2回繰り返し

# 連符とタイの組み合わせ
sine-mml play "{CDE}3 G4&8"
# 3連符の後にタイで連結した音符
```
...
# エラー: 連符数が範囲外
sine-mml play "{CDE}100"

# エラー: 6階層以上のネスト
sine-mml play "{{{{{{C}2}2}2}2}2}2"
...
v3.0で追加されたMIDIストリーミング機能を使用すると、MMLを外部MIDIデバイスやDAWにリアルタイム送信できます。（v3.0新機能）

### 概要

MIDIストリーミングを使うと、音声合成ではなく、接続されたMIDIデバイス（シンセサイザー、DAWなど）で音を鳴らすことができます。

### オプション

| オプション | 説明 | デフォルト |
|-----------|------|-----------|
| `--midi-out <DEVICE>` | MIDIデバイスIDまたは名前 | - |
| `--midi-channel <1-16>` | MIDIチャンネル | 1 |
| `--midi-list` | 利用可能なMIDIデバイス一覧を表示 | false |
...
# デバイス名で指定
sine-mml play "CDEFGAB" --midi-out "IAC Driver"

# チャンネル10（ドラムトラック）に出力
sine-mml play "CDEFGAB" --midi-out 0 --midi-channel 10

# 高テンポでMIDI出力
sine-mml play "T180 L8 CDEFGAB" --midi-out 0
```
...
| 制限 | 説明 |
|-----|------|
| MIDIチャンネル | 1〜16の範囲（0や17以上はエラー） |
| MIDIデバイス | システムで利用可能なデバイスのみ |
| 同時使用 | `--midi-out` と波形オプション（`--waveform`）は排他的ではない |
...
```bash
# エラー例
sine-mml play "CDEF" --midi-out 99
# → [MML-E018] 無効なMIDIデバイスID: 99
```
...
```bash
# エラー例
sine-mml play "CDEF" --midi-out 0 --midi-channel 0
# → [MML-E024] 無効なMIDIチャンネル: 0（1-16の範囲で指定）
```
...
```bash
sine-mml play "T120 L4 O4 C {DEF}3 G {ABC}3 >C2"
```
...
**対処法**:
- 開き括弧 `{` と閉じ括弧 `}` が対応しているか確認
- 連符数が2〜99の範囲内か確認（1は無効）
- ネスト深度が5階層以内か確認

### MIDIエラー（v3.0新機能）

**原因**: MIDIデバイスが見つからない、または設定が不正

**対処法**:
- `--midi-list` でデバイス一覧を確認
- デバイスIDまたは名前が正しいか確認
- MIDIチャンネルが1〜16の範囲内か確認
